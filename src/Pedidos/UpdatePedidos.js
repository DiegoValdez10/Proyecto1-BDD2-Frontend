...
function UPedidos() {
    ...
    const UpdatePedido = async () => {
        try {
            const response = await axios.put(`http://localhost:3161/Pedidos/${pedido.ID_pedido}`, pedido);
            console.log(response.data);
            // Aquí podrías redirigir a otra página o hacer cualquier otra acción después de actualizar el pedido
        } catch (error) {
    ...
    return (
        <div className="pedidos-container">
            <h1 className="pedidos-title">Actualizar pedido</h1>
            ...
            <div className="pedidos-buttons">
                <button onClick={UpdatePedido}>Actualizar pedido</button>
            </div>
            <div className="arrow-container">
                <Link to="/ver-Pedidos">
                    <button className="arrow-button">&#8592;</button>
                </Link>
                <Link to="/borrar-Pedidos">
                    <button className="arrow-button">&#8594;</button>
                </Link>
            </div>
        </div>
